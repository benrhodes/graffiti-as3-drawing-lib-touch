...
package com.nocircleno.graffiti.tools.brushes
{
   ...
   public class DiamondBrush extends Brush implements IBrush
   {
      ...
      /**
       * The <code>DiamondBrush</code> constructor.
       * 
       ...
       */
      public function DiamondBrush(brushSize:Number = 4, brushColor:uint = 0x000000, brushAlpha:Number = 1, bitmapCacheRenderMode:String = null)
      {
      ...
      {         
         ...
         for(var i:int=0; i<_numberInteractions; i++)
         {
            ...
            if(Math.abs(_previousPointRef.x - _nextPointRef.x) > Math.abs(_previousPointRef.y - _nextPointRef.y))
            {   
               commands.push(GraphicsPathCommand.MOVE_TO);
               drawingData.push(_drawPoint1.x);
               drawingData.push(_drawPoint1.y - _halfSize);
               
               commands.push(GraphicsPathCommand.LINE_TO);
               drawingData.push(_drawPoint2.x);
               drawingData.push(_drawPoint2.y - _halfSize);
               
               commands.push(GraphicsPathCommand.LINE_TO);
               drawingData.push(_drawPoint2.x + _halfSize);
               drawingData.push(_drawPoint2.y);
               ...
               drawingData.push(_drawPoint1.y);
            ...
            else
            {
               ...
               if(_nextPointRef.y <= _previousPointRef.y) {
                  _drawPoint1 = _nextPointRef;
                  _drawPoint2 = _previousPointRef;
               } else {
                  _drawPoint1 = _previousPointRef;
